...
class LineNumberHandler extends DefaultHandler {
...
class ClassLoaderXMLParser {
    ...
    static {
        ...
        String singleLoadMode = p.getProperty("com.alibaba.quickstart.singleLoaderMode");
        if (singleLoadMode != null && singleLoadMode.equalsIgnoreCase("true")) {
    ...
    protected static void parse() throws Exception {
        final SAXParserFactoryImpl factory = new SAXParserFactoryImpl();
        SAXParserImpl parser = (SAXParserImpl)factory.newSAXParser();
        final DocumentBuilderFactory docBuilderFactory = new DocumentBuilderFactoryImpl();
        ...
        for (int i = 0; i < classloaders.getLength(); i++) {
            ...
            for (int j = 0; j < elems.getLength(); j++) {
                ...
                if (loaderInner.getNodeType() == Node.ELEMENT_NODE) {
                    if (loaderInner.getFirstChild() == null) {
                        continue;       // some like "<uniqueName></uniqueName>"
                    }
                    if ("klass".equals(loaderInner.getNodeName())) {
                        klass = loaderInner.getFirstChild().getNodeValue();
                    } else if ("fieldName".equals(loaderInner.getNodeName())) {
                        fieldName = loaderInner.getFirstChild().getNodeValue();
                    } else if ("appendJarPathsAsName".equals(loaderInner.getNodeName())) {
                        appendJarPathsAsName = Boolean.parseBoolean(loaderInner.getFirstChild().getNodeValue());    // all other value except 'true' will be false.
                    } else if ("uniqueName".equals(loaderInner.getNodeName())) {
        ...
        if (identifier != null) {
            ...
            try {
                ...
                System.out.println("[Name] Register [" + loader + "] as [" + identifier + "]: "
                        + "hash [" + Integer.toHexString((int) AJDKUtils.calculateSignatureForName.invoke(null, identifier)) + "] "
                        + (!specialSingleClassLoaderMode ? "(normal mode)" : "(single loader mode)"));
            } catch (Exception e) {
