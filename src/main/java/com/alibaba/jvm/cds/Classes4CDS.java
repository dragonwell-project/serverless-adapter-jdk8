...
//                System.out.println("Extracted file: " + tmpDir + " " + fatJarName);
    ...
    public class CDSData {
        ...
        public String originSource; // originSource != source when it's a fat jar.
    ...
    void printCDSData(CDSData data) {
        ...
        if (data.source != null && !data.source.contains("jrt:")) {
            ...
            out.print("origin: " + data.originSource); out.print(" ");
            out.print("source: " + data.source); out.print(" ");
            // System.out.println(data.source);
        } else {
            out.print("origin: " + data.originSource); out.print(" ");   // Well... one loaded by AppClassLoader may not have "source"... because it's is_builtin.
        }
    ...
    List<CDSData> all = new ArrayList<>();
    List<CDSData> allNotFound = new ArrayList<>();

    // This is a workaround for the feature NotFound:
    // Some services, for example, Ultron2, would record ClassNotFoundExceptions and throw them in the end of the process.
    //
    final Set<String> notFoundBlackList = new HashSet<String>() {{
        add("com/alibaba/china/mmc/business/order/render/redCoupon/MmcRedEnvelopeQueryParamAttach");
        add("com/alibaba/china/mmc/business/order/render/redCoupon/MmcRedEnvelopeSubmitParamAttach");
        add("com/alibaba/china/mmc/business/order/render/paysign/MmcPaySignQueryAttach");
        add("com/alibaba/china/mmc/business/order/render/promotion/MMCPromotionCOAttachQueryParamsSpi");
        add("com/alibaba/china/mmc/business/order/render/promotion/MMCPromotionCOAttachSubmitParamsSpi");
    }};
        ...
        try {
            ...
            while (!line.isEmpty()) {
                if (line.contains("defining_loader_hash:") && line.contains("initiating_loader_hash:")) {
                    ...
                    // a filter.
                    if (!notFoundBlackList.contains(name)) {
                        String source = getKlassPath(line);
                        String initiatingHash = getInitiatingLoaderHash(line);
                        if (!notFoundSet.contains(name + initiatingHash)) {
                    ...
                    String name = getKlassName(line);
                    String id = getId(line);
                    String originalSource = getOriginalSource(line);
                    String source = getKlassPath(line);
