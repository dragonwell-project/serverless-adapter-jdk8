...
CLASS4CDS=com.alibaba.jvm.cds.Classes4CDS
LST=hello2.lst
JSA=hello2.jsa
LST_AFTER=hello.lst
JAR="-jar ${FULLPATH}/spring-helloworld.jar"
OP="-DstartupBenchmark -XX:+UnlockExperimentalVMOptions -XX:+UnlockDiagnosticVMOptions -XX:+EagerAppCDS -XX:+EagerAppCDSLegacyVerisonSupport" # TODO: remove "Eager" prefix
OP="${OP} -agentlib:loadclassagent -Xbootclasspath/a:${AGENT}"
#OP="${OP} -Xlog:class+eagerappcds=trace"
rm -rf ${LST} ${JSA} ${LST_AFTER} tmp

${JAVA_HOME}/bin/java ${OP} -Xshare:off -XX:DumpLoadedClassList=${LST} ${JAR}
${JAVA_HOME}/bin/java -cp ${AGENT} ${CLASS4CDS} ${LST} ${LST_AFTER}
